...
2. Read PR description and commit messages to understand the intent.
...
- Deliver the review and test results from step 4 only. Do not change the code, commit, push, merge, or create PRs unless the user explicitly asks (`AGENTS.md`).
...
- [ ] External command calls handle failures per coding guidelines
- [ ] New environment variables use the `LEAPP_` prefix and `get_env` per coding guidelines
...
- [ ] No spelling errors in commit messages or PR description point to `leapp-commit-message-assist` for rewording
...
- [ ] Fixups are squashed before review (suggest autosquash rebase if not)
...
- [ ] Tests verify produced messages and reports (type, fields, inhibitor groups) across relevant scenarios.
