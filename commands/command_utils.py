...
def _retrieve_os_release_contents(_os_release_path='/etc/os-release', strip_double_quotes=True):
    ...
    def strip_quotes_from_field_value(key_val_pair):
        return (key_val_pair[0], key_val_pair[1].strip('"'))
    ...
    with open(_os_release_path) as os_release_handle:
        ...
        for line in lines:
            ...
            key_val_pair = line.strip().split('=', 1)

            if strip_double_quotes:
                key_val_pair = strip_quotes_from_field_value(key_val_pair)

            os_release_contents[key_val_pair[0]] = key_val_pair[1]
