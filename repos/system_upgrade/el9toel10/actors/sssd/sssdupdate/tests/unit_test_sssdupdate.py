...
def mock_filesystem(monkeypatch):
    ...
    def mocked_close(fd: int):
        mocked_filesystem.close(fd)
...
def make_mock_file(contents: str) -> str:
    name = str(Path('/tmp') / f'file.{str(random.uniform(1, 999999))}')
    file = mocked_filesystem.open(name, 'w')
    file.write(contents)
    file.close
...
def check_file(name:str, expected: str):
    assert name in mocked_filesystem._by_name.keys()
