...
def _fix_local_fs_requires(dir_containing_mount_units):
    ...
    for unit_file in os.listdir(dir_containing_mount_units):
        ...
        try:
            ...
            err = 'Failed to required unit dependencies for local-fs.target in upgrade initramfs.'
            details = {'details': str(err)}
            raise StopActorExecutionError(err, details=details)
...
def remove_units_for_targets_that_are_already_mounted_by_dracut(dir_with_our_mount_units):
    ...
    '''
    already_mounted_units = ['-.mount', 'usr.mount']
    ...
    for unit in already_mounted_units:
        ...
        if not os.path.exists(unit_location):
            api.current_logger().debug('The {} unit does not exists, although it should.'.format(unit))
            continue

        os.unlink(unit_location)
...
def request_units_inclusion_in_initramfs(files_to_include):
    ...
    additional_files = [
        '/usr/bin/swapon'  # If the system has swap, we have also generated a swap unit to activate it
    ]
