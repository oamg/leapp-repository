...
def mock_cmd_output():
    ...
    mocked_cmd = 'BOOT_IMAGE=(hd0,msdos1)/vmlinuz-xxx root=UUID=some_uid ro console=tty0 '\
...
def test_cmdline_content(monkeypatch):
    ...
    expected_cmd_output = 'BOOT_IMAGE=(hd0,msdos1)/vmlinuz-xxx root=UUID=some_uid ro console=tty0 '\
        'console=ttyS0,115200 rd_NO_PLYMOUTH biosdevname=0 net.ifnames=0 crashkernel=auto'
...
def test_cmdline_run_failed(monkeypatch):

    def run_mocked_error(cmd, **kwargs):
        assert cmd == ['cat', '/proc/cmdline']
        raise CalledProcessError("Mocked error raised", cmd, "result")
