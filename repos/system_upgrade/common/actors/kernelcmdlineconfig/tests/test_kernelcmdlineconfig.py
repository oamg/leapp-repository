...
def test_kernelcmdline_explicit_configs(monkeypatch):
    ...
    grubby_remove = ['grubby', '--update-kernel=/boot/vmlinuz-X', '--remove-args', 'key1=value1']
    expected_cmds = [
        grubby_remove + ['-c', '/boot/grub2/grub.cfg'],
        grubby_remove + ['-c', '/boot/efi/EFI/redhat/grub.cfg'],
        grubby_cmd_info,
