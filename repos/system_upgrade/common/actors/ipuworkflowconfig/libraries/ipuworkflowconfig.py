...
def extract_paths_upgrade_paths_for_distro_and_flavour(all_definitions, distro_id, flavour):
...
def construct_virtual_versions(all_upgrade_path_defs, distro_id, source_version, target_version):
    ...
    if not centos_upgrade_paths:
        raise StopActorExecutionError('There are no upgrade paths are defined for CentOS.')
...
def produce_ipu_config(actor):
    ...
    raw_upgrade_paths = extract_paths_upgrade_paths_for_distro_and_flavour(all_upgrade_path_defs,
