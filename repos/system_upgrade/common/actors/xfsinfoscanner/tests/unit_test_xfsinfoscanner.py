...
from leapp.models import FstabEntry, MountEntry, StorageInfo, SystemdMountEntry, XFSInfo, XFSInfoFacts, XFSPresence
...
def test_scan_xfs_ignored_xfs(monkeypatch):
    ...
    assert xfs_info_facts.mountpoints[0].meta_data == TEST_XFS_INFO_FTYPE1_PARSED['meta-data']
    assert xfs_info_facts.mountpoints[0].data == TEST_XFS_INFO_FTYPE1_PARSED['data']
    assert xfs_info_facts.mountpoints[0].naming == TEST_XFS_INFO_FTYPE1_PARSED['naming']
    assert xfs_info_facts.mountpoints[0].log == TEST_XFS_INFO_FTYPE1_PARSED['log']
    assert xfs_info_facts.mountpoints[0].realtime == TEST_XFS_INFO_FTYPE1_PARSED['realtime']
...
def test_scan_xfs_without_ftype(monkeypatch):
    ...
    assert xfs_info_facts.mountpoints[0].meta_data == TEST_XFS_INFO_FTYPE0_PARSED['meta-data']
    assert xfs_info_facts.mountpoints[0].data == TEST_XFS_INFO_FTYPE0_PARSED['data']
    assert xfs_info_facts.mountpoints[0].naming == TEST_XFS_INFO_FTYPE0_PARSED['naming']
    assert xfs_info_facts.mountpoints[0].log == TEST_XFS_INFO_FTYPE0_PARSED['log']
    assert xfs_info_facts.mountpoints[0].realtime == TEST_XFS_INFO_FTYPE0_PARSED['realtime']
...
def test_scan_xfs_no_message(monkeypatch):
    ...
    xfs_info_facts = next(model for model in api.produce.model_instances if isinstance(model, XFSInfoFacts))
    assert len(xfs_info_facts.mountpoints) == 0
