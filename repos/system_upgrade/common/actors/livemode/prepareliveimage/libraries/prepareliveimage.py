...
def create_fstab_mounting_current_root_elsewhere(context, host_fstab):
    ...
    mountpoints present in source system's fstab will be made relative to SOURCE_ROO_MOUNT_LOCATION.
    ...
    for fstab_entry in host_fstab:
        ...
        if fstab_entry.fs_vfstype not in ('xfs', 'ext4', 'ext3', 'vfat'):
        ...
        new_mountpoint = os.path.join(relative_root, fstab_entry.fs_file.rstrip('/'))
...
def setup_console(context):
    ...
    try:
        ...
        if os.path.exists(tty1_service_symlink):
            context.remove(tty1_service_symlink)
...
def setup_sshd(context, authorized_keys):
    ...
    try:
        ...
        for key in hostkeys:
            ...
            os.chmod(key_path, access_rights)
            os.chown(key_path, uid=0, gid=grp.getgrnam(group).gr_gid)
    except OSError as error:
    ...
    if get_target_major_version() == '8':  # set to LEGACY for 7>8 only
        try:
            ...
            raise StopActorExecutionError('TEMPORARY: Failed to set cryptopolicies')
...
def prepare_live_image(userspace, storage, boot, livemode):
    ...
    config = next(api.consume(LiveModeConfigFacts), None)
    if not config:
    ...
    with mounting.NspawnActions(base_dir=userspace_info.path) as context:
        ...
        enable_dbus(context)
        setup_network(context, livemode.nm)
