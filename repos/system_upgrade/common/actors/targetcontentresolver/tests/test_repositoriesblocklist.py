...
def test_compute_blocklist_merges_internal_and_external(monkeypatch):
    ...
    RepositoriesSetupTasks.blocklist and produces both messages.
    """
    external_tasks = [
        RepositoriesSetupTasks(to_enable=['some-repo'], blocklist=['ext-blocked-1', 'ext-blocked-2']),
    ]
    monkeypatch.setattr(
...
def test_compute_blocklist_multiple_external_tasks(monkeypatch):
    ...
    external_tasks = [
        RepositoriesSetupTasks(blocklist=['ext-1'], to_enable=['repo-a']),
        RepositoriesSetupTasks(blocklist=['ext-2', 'ext-3'], to_enable=['repo-b']),
    ]
    monkeypatch.setattr(
        repositoriesblocklist, 'get_blocklisted_repoids', lambda _repo_map: set()
    )
