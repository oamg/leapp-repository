...
def test_EFIBootLoader__efi_path_to_canonical():
    assert grub.EFIBootLoader._efi_path_to_canonical(r'\EFI\redhat\shimx64.efi') == '/boot/efi/EFI/redhat/shimx64.efi'
...
def test_canonical_to_efi_to_canonical():
    ...
    assert grub.EFIBootLoader._efi_path_to_canonical(efi) == canonical


def test_efi_path_to_canonical_to_efi():
    efi = r'\EFI\redhat\shimx64.efi'
    canonical = grub.EFIBootLoader._efi_path_to_canonical(efi)
...
def test_EFIBootLoader_is_referring_to_file(efi_bin_source, expected):
    bootloader_entry = grub.EFIBootLoader('0001', 'Redhat', False, efi_bin_source)
...
def test_EFIBootLoader_get_canonical_path(efi_bin_source, expected):
    bootloader_entry = grub.EFIBootLoader('0001', 'Redhat', False, efi_bin_source)
