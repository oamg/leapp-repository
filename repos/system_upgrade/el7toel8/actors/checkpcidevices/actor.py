...
class CheckPCIDevices(Actor):
    ...
    description = 'Check PCI devices and inhibit upgrade process if any unsupported one found.'
    consumes = (PCIDevices,)
                ...
                if 'SCSI' in device.cls and 'LSI Logic' in device.vendor:
                    self.produce(CheckResult(
                        ...
                        details='Kernel driver necessary for LSI Logic SCSI Controller (mpt*) '
                                'is not available at Red Hat Enterprise Linux 8. Since this '
                                'would prevent controller of working properly upgrade process '
                                'will be inhibited.',
