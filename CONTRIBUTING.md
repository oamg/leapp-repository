...
1. All PRs should have the appropriate testcases when possible. For example, bug fixes could include tests that demonstrates the issue w/o your fix.
2. All PRs must have the appropriate documentation about changes made within the same PR. Note, not all PRs will necessarily require a documentation change but if it does please include it in the same PR so the PR is complete and not just a partial solution.
3. All PRs must have appropriate documentation. New features should be described, and an example of use provided when relevant.
4. All PRs should follow Code Guidelines
