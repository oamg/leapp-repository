...
Leapp features an experimental feature named _livemode_ that partially replaces
...
`LEAPP_DEVEL_ENABLE_LIVE_MODE=1` and running leapp as `leapp upgrade --enable-experimental-feature livemode`.
```
LEAPP_DEVEL_ENABLE_LIVE_MODE=1 LEAPP_UNSUPPORTED=1 leapp upgrade --enable-experimental-feature livemode
```
