...
Due to some missing initial variables, which usually are set by the framework, those vars need to be exported manually. Note that following vars are example ones, adjust them to your needs depending on your system configuration:
```shell
...
```shell
snactor run ipu_workflow_config --print-output 
```
...
```{note}
Point to `leapp.conf` file with *--config* option. By default this file is located in `/etc/leapp/` and  among others, it contains Leapp database (`leapp.db`) localisation. When working with given database, either adjust configuration file or place database file in default localisation.
```
